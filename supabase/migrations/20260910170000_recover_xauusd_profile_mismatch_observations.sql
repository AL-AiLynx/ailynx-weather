...
-- projection only for the audited XAUUSD profile-snapshot mismatch.
begin;

alter table public.as1_recovered_observations
  drop constraint as1_recovered_observations_asset_check,
  drop constraint as1_recovered_observations_ticker_id_check,
  drop constraint as1_recovered_observations_venue_check,
  drop constraint as1_recovered_observations_symbol_check,
  drop constraint as1_recovered_observations_recovered_source_profile_code_check;
...
-- This insert is deliberately narrow and idempotent. Rows qualify only when
-- the old BTC profile snapshot was the sole validation failure.
with candidates as (
  select
    raw.*,
  ...
  where raw.satellite_id = 'AS1'
    ...
    and raw.ticker_id = 'OANDA:XAUUSD'
    and raw.venue = 'OANDA'
    and raw.symbol = 'XAUUSD'
    ...
    and raw.raw_envelope #>> '{instrument,ticker_id}' = 'OANDA:XAUUSD'
    and raw.raw_envelope #>> '{instrument,venue}' = 'OANDA'
    and raw.raw_envelope #>> '{instrument,symbol}' = 'XAUUSD'
