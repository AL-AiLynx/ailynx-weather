...
import {validateSourceProfile} from "./source-profiles.ts";
...
export type ValidatedIngestRequest = {
  kind: "single" | "bundle";
  schema_version: string;
  request_event_key: string;
  rows: IngestRow[];
  total_frames: number;
...
const BUNDLE_CODE_VERSION = "NOT_PROVIDED_BY_BUNDLE_V0_1";
const BUNDLE_SOURCE_PROFILE_CODE = "CB_BTCUSD_SPOT_20260722_V1";
...
function requireNullableFiniteNumber(parent: JsonRecord, key: string): number | null {
  if (!Object.hasOwn(parent, key)) throw new ValidationError(`${key} is required`);
  const value = parent[key];
  if (value === null) return null;
  if (typeof value !== "number" || !Number.isFinite(value)) throw new ValidationError(`${key} must be a finite number or null`);
  return value;
}
...
function requireStringArray(parent: JsonRecord, key: string): string[] {
  ...
  if (!Array.isArray(value) || value.some((item) => typeof item !== "string")) {
    throw new ValidationError(`${key} must be a string array`);
  }
...
export function validateEnvelope(envelopeValue: unknown): IngestRow {
  ...
  validateSourceProfile(envelope, sourceProfileCode, layoutId);
...
function validateBundleQuality(value: JsonRecord): BundleQuality {
  const sensorQuality = requireNullableFiniteNumber(value, "sensor_quality");
  const valid = requireBoolean(value, "valid");
  const flags = requireStringArray(value, "flags");
  return {sensorQuality, valid, flags, raw: value};
}

function ledgerSensorQuality(quality: BundleQuality): string {
  if (!quality.valid) return "INVALID";
  if (quality.sensorQuality === null) return "WATCH";
  if (quality.sensorQuality >= 100) return "GOOD";
  if (quality.sensorQuality > 0) return "LIMITED";
  return "WATCH";
}

function withoutFrames(envelope: JsonRecord): JsonRecord {
  const metadata: JsonRecord = {};
  for (const [key, value] of Object.entries(envelope)) {
    if (key !== "frames") metadata[key] = value;
  }
  return metadata;
}

function validateBundleEnvelope(envelope: JsonRecord): ValidatedIngestRequest {
  const schemaVersion = requireString(envelope, "schema_version");
  requireLiteral(envelope, "schema_version", "as1.bundle.v0.1");
  requireLiteral(envelope, "satellite_id", "AS1");
  requireLiteral(envelope, "platform", "TRADINGVIEW");
  requireLiteral(envelope, "layout_id", "HORUS_B");
  requireLiteral(envelope, "sensor", "GENUT_B");
  requireLiteral(envelope, "packet_type", "MTF_BAR_CLOSE_BATCH");

  const satelliteId = requireString(envelope, "satellite_id");
  const platform = requireString(envelope, "platform");
  const layoutId = requireString(envelope, "layout_id");
  const sensor = requireString(envelope, "sensor");
  const packetType = requireString(envelope, "packet_type");
  const batchEventKey = requireString(envelope, "batch_event_key");
  requireEpochMs(envelope, "emitted_at");

  const instrument = requireRecord(envelope, "instrument");
  const tickerId = requireString(instrument, "ticker_id");
  const venue = requireString(instrument, "venue");
  const symbol = requireString(instrument, "symbol");
  requireLiteral(instrument, "asset_class", "CRYPTO");
  requireLiteral(instrument, "symbol", "BTC");
  requireLiteral(instrument, "quote", "USD");
  requireLiteral(instrument, "venue", "COINBASE");
  requireLiteral(instrument, "market_type", "SPOT");
  requireLiteral(instrument, "ticker_id", "COINBASE:BTCUSD");
  requireRecord(envelope, "session_hint");
  const bundleQuality = validateBundleQuality(requireRecord(envelope, "quality"));

  const frames = envelope.frames;
  if (!Array.isArray(frames) || frames.length < 1) throw new ValidationError("frames must be a non-empty array");

  const bundleMetadata = withoutFrames(envelope);
  ...
  for (let index = 0; index < frames.length; index += 1) {
    ...
    if (!isRecord(frameValue)) throw new ValidationError(`frames[${index}] must be an object`);
    const frame = frameValue;
    const timeframe = requireString(frame, "tf");
    requireInteger(frame, "tf_minutes", 1);
    const barOpenTime = requireEpochMs(frame, "bar_open_at");
    const barCloseTime = requireEpochMs(frame, "bar_close_at");
    const observedAt = requireEpochMs(frame, "observed_at");
    if (barCloseTime <= barOpenTime) throw new ValidationError("bar_close_at must be after bar_open_at");
    const frameEventKey = requireString(frame, "frame_event_key");
    const state = requireRecord(frame, "state");
    const values = requireRecord(frame, "values");
    const frameQuality = validateBundleQuality(requireRecord(frame, "quality"));

    if (eventKeys.has(frameEventKey)) {
      duplicateFrames += 1;
      continue;
    }
    eventKeys.add(frameEventKey);

    const normalizedQuality = ledgerSensorQuality(frameQuality);
    const rowPayload: JsonRecord = {
      batch_event_key: batchEventKey,
      frame_event_key: frameEventKey,
      state,
      values,
      frame_quality: frameQuality.raw,
      bundle_quality: bundleQuality.raw,
      bundle_metadata: bundleMetadata,
    };
    const rawEnvelope: JsonRecord = {
      ...bundleMetadata,
      observer: sensor,
      code_version: BUNDLE_CODE_VERSION,
      source_profile_code: BUNDLE_SOURCE_PROFILE_CODE,
      timing: {
        timeframe,
        bar_open_time: barOpenTime,
        bar_close_time: barCloseTime,
        sent_at: observedAt,
      },
      confirmed: true,
      quality: {sensor_quality: normalizedQuality, valid: frameQuality.valid, flags: frameQuality.flags},
      client_event_key: frameEventKey,
      payload: rowPayload,
    };
    ...
    rows.push({
      ...
      bar_close_time: barCloseTime,
      sent_at: observedAt,
      bar_duration_ms: barCloseTime - barOpenTime,
      packet_type: packetType,
      confirmed: true,
      sensor_quality: normalizedQuality,
      valid: frameQuality.valid,
      flags: frameQuality.flags,
      client_event_key: frameEventKey,
  ...
  return {
    kind: "bundle",
    schema_version: schemaVersion,
    request_event_key: batchEventKey,
    rows,
...
export function validateIngestRequest(envelopeValue: unknown): ValidatedIngestRequest {
  if (isRecord(envelopeValue) && typeof envelopeValue.schema_version === "string" && envelopeValue.schema_version.startsWith("as1.bundle.")) {
    if (envelopeValue.schema_version !== "as1.bundle.v0.1") throw new ValidationError("unsupported schema_version");
    return validateBundleEnvelope(envelopeValue);
  }

  const row = validateEnvelope(envelopeValue);
  return {
    kind: "single",
    schema_version: row.schema_version,
    request_event_key: row.client_event_key,
    rows: [row],
    total_frames: 1,
    duplicate_frames: 0,
  };
