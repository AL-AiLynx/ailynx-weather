...
test("history metrics use only ordered valid snapshots from one timeframe", () => { const engine = globalThis.AiLynxWeatherEngine; const history = [{timeframe: "4H", score: 40, state: "CLOUDY", valid: true, noise: 30}, {timeframe: "4H", score: 52, state: "CLOUDY", valid: true, noise: 30}]; assert.equal(engine.computeChangeRate(history), 12); assert.equal(engine.computeDurability(history), 88); assert.equal(engine.computeChangeRate([{...history[0]}, {...history[1], timeframe: "8H"}]), null); });
