...
test("dashboard provides a config-driven Lynx timeframe board without invented metrics", async () => {
  ...
  assert.match(worker, /ailynx-weather-v29/);
  ...
  assert.doesNotMatch(html, /주요 구간|주요 기간/);
  ...
  assert.match(app, /weatherMetricText\(durability, "persistence"\)/);
  assert.match(app, /weatherMetricText\(changeRate, "changeRate"\)/);
