...
- `AS1_V13_CLOSED_001`: all four AS1 layouts are explicitly represented as NOT_EXPECTED, with an empty expected/missing set and a null score.
...
Normalized-to-Fusion provenance includes `expectation_state` from `session_context` and `received_at` from `server_evaluation`, in addition to identity, timing, quality, freshness, contract, and dedup fields.
...
The validator does not trust stored visibility breakdown counts. It independently recalculates expected, not-expected, fresh, aging, stale, missing, and invalid counts from the summary sets and selected component states for every fixture.
